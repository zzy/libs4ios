...
#pragma mark NodeData type routines
...
#pragma mark NodeData recovery routines

// Return an array of child keys. No recursion
- (NSArray *) keys
{
    NSMutableArray *results = [NSMutableArray array];
    for (NodeData *node in self.children) [results addObject:node.key];
    return results;
}
...
{
    NSMutableArray *results = [NSMutableArray array];
    for (NodeData *node in self.children) if (node.leafvalue) [results addObject:node.leafvalue];
    return results;
}
...
#pragma mark NodeData search and retrieve routines
...
{
    NSMutableArray *result = [NSMutableArray array];
    for (NodeData *node in [self objectsForKey:aKey])
        if (node.leafvalue)
...
- (NodeData *) objectForKeys: (NSArray *) keys
{
...
{
    NodeData *node = [self objectForKeys:keys];
    return node.leafvalue;
}
...
- (void) teardown
{
    for (NodeData *node in [[self.children copy] autorelease]) [node teardown];
